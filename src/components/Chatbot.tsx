...
import { askForensecAI } from '../ai/forensecAI.ts';
...
const Chatbot: React.FC = () => {
  ...
  useEffect(() => {
    if (messagesContainerRef.current && messages.length > 1) {
      ...
      // Use scrollTo when available; fall back for environments like jsdom.
  ...
  const handleSendMessage = async (e?: React.FormEvent, quickQuery?: string) => {
    ...
    try {
      ...
      // Call Gemini AI
      const aiReply = await askForensecAI(query);
      ...
      const errorMessage: Message = {
        id: (Date.now() + 1).toString(),
        text: "Forensic AI system error occurred.",
        sender: 'bot',
        timestamp: new Date(),
      };
              ...
              {suggestions.length > 0 && (
                ...
                >
                  <div className="flex items-center gap-1 mb-2">
                    ...
                    <span className="text-xs text-text-secondary font-medium">Suggestions:</span>
                  </div>
