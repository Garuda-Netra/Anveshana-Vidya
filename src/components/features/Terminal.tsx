...
const BUILTIN_COMMANDS = ['help', 'helpcat', 'glossary', 'workflow', 'clear', 'explain'];
...
export default function Terminal() {
  ...
  const allCommands = useMemo(() => {
    ...
    const combined = new Set([...enhancedNames, ...basicNames, ...LAB_COMMANDS, ...LEARNING_COMMANDS, ...BUILTIN_COMMANDS]);
  ...
  useEffect(() => {
    if (!hasMountedRef.current) {
      ...
      // Auto-focus the input on initial mount so the user can type immediately
  ...
  // Handlers
  const focusInput = () => {
    if (!isPaletteOpen) inputRef.current?.focus({ preventScroll: true });
  };
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter') {
      ...
      if (activeSuggestionIndex >= 0 && suggestions.length > 0) {
        // Execute the highlighted suggestion immediately instead of just filling it in
                ...
                >
                  {suggestions.map((s, idx) => (
                    <div 
                      ...
                      onClick={() => {
                        // Execute the command directly on click rather than just filling the input
                        setSuggestions([]);
                        setInput('');
                        setActiveSuggestionIndex(-1);
                        handleCommand(s);
