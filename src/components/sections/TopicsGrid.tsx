...
function getTopicAsset(assets3D: string[] | string | null | undefined): string | null {
  if (!assets3D) return null;
  if (Array.isArray(assets3D)) return assets3D.length > 0 ? assets3D[0] : null;
  if (typeof assets3D === 'string') return assets3D;
  return null;
}
    ...
    <section id="topics" className="py-24 px-4 relative min-h-screen flex flex-col justify-center">
      <div className="max-w-7xl mx-auto w-full">
        <SectionHeader 
          ...
          subtitle="Explore key areas of digital forensics through our comprehensive learning modules with interactive 3D visualizations."
        />
        
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {topics.map((topic) => {
            const asset3D = getTopicAsset(topic.assets3D);
                    ...
                    <div className="flex gap-2">
                      {asset3D && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            setSelectedModel({ title: topic.title, asset: asset3D });
                          }}
                          className="p-1.5 bg-accent-neon/10 hover:bg-accent-neon/20 text-accent-neon rounded transition-colors"
                          title="View 3D Model"
                      ...
                      <button
                        ...
                        className="p-1.5 hover:bg-accent-neon/20 rounded transition-colors"
      ...
      >
        ...
        <div className="mt-4 text-text-secondary text-sm">
          <p>Interactive 3D Visualization. Use mouse to rotate, zoom, and inspect internal structures.</p>
        </div>
      </Modal>
      ...
      >
        <div className="space-y-6">
          ...
          {getTopicAsset(selectedTopic?.assets3D) && (
            <button
              onClick={() => {
                setSelectedModel({ title: selectedTopic!.title, asset: getTopicAsset(selectedTopic?.assets3D)! });
